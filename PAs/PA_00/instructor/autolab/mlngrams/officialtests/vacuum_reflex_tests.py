""" Unit tests for vacuum cleaner world
...
class TestNGrams(unittest.TestCase):
    ...
    # Bigram Tests ------------
    @points_decor(points=15, autograder_name='bigrams')
    def test_calculate_bigrams(self):
        bigrams = text_gen.calculate_bigrams(['i', 'think', 'therefore', 
                                               'i', 'am', 'i', 'think', 
                                               'i', 'think'])
        msg = 'Bigram distribution did not match'
        self.assertEqual(bigrams, self.bigrams, msg)
    ...
    def test_bigram_text_correct_length(self):
        ...
        msg = 'random_bigram_text did not generate correct length result'
        self.assertEqual(len(text.split()), 10, msg)

    @points_decor(points=10, autograder_name='bigrams')
    def test_bigram_text_correct_start(self):
        text = text_gen.random_bigram_text('i', self.bigrams, 10)
        self.assertEqual(text.split()[0], 'i',
            'random_bigram_text did not generate correct first word')
    ...
    def test_bigram_text_possible(self):
        text = text_gen.random_bigram_text('i', self.bigrams, 100)
        words = text.split()
        prev = 'i'
        msg = 'string generated by random_bigram_text generate is not possible'
        msg += ':' + text
        for i in range(1, len(words)):
            current = words[i]
            self.assertGreaterEqual(self.bigrams[prev][current], 0, msg)
            prev = current

    # Trigram Tests ------------

    @points_decor(points=3, autograder_name='trigrams')
    ...
    @points_decor(points=2, autograder_name='trigrams')
